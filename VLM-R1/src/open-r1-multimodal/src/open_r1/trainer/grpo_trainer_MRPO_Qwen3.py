...
class VLMGRPOTrainer_MRPO_Qwen3(Trainer):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Optional map for process_reward to access gold label reasonings by (image_name, question)
        ...
        # Note: use_cache is set on config after model loading, not as init kwarg
        ...
        # Disable output_hidden_states to speed up inference on CUDA
        ...
        if self.beta == 0.0:
            ...
            # Use the same VLM class for the reference model to support VLM configs (e.g., Qwen2.5-VL)
        ...
        self.generation_config = GenerationConfig(
            ...
            use_cache=True,  # Enable cache for proper generation
            return_dict_in_generate=False,
            output_hidden_states=False,  # Disable to speed up inference on CUDA
        )
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            ...
            # Temporarily enable use_cache for faster generation (disabled during training for gradient checkpointing)
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                # Attach global reasoning map for reward functions that need it
        ...
        # Build per-token advantage multiplier (+1 for correct sentence, -1 for incorrect), if process scores provided
        token_adv_multiplier = None
        if len(total_score_per_step_list) > 0:
            # Use the first process reward's scores
            process_scores_per_sample = total_score_per_step_list[0]  # list of list per sample
            # Select tokenizer for sentence tokenization
            tokenizer = getattr(self.processing_class, "tokenizer", None)
            if tokenizer is None and isinstance(self.processing_class, PreTrainedTokenizerBase):
                tokenizer = self.processing_class
            # Initialize multiplier as ones
            token_adv_multiplier = torch.ones_like(completion_mask, dtype=torch.float32, device=device)
            
            
            # Return (sentence, start_char, end_char) for sentences inside <thinking> tags (absolute spans in comp_text)
            def split_think_sentences_with_spans(text: str) -> list[tuple[str, int, int]]:
                ...
                # Only consider the text inside <thinking>...</thinking> tags
                for m in re.finditer(r"<thinking>([\s\S]*?)</thinking>", text, flags=re.IGNORECASE):
                    inner = m.group(1)
                    base = m.start(1)  # Absolute position of <thinking> content start
                    
                    # Stop at <answer> tag if present inside <thinking> block
            ...
            for row_idx, (comp_text, comp_ids) in enumerate(zip(completions, completion_ids)):
                ...
                if tokenizer is None:
                    # Cannot map tokens to sentences without a tokenizer
                    continue
                # Normalize comp_text if conversational wrapped
                if isinstance(comp_text, list):
                    # [[{"role": "assistant", "content": "..."}]] -> take content
                    try:
                        comp_text = comp_text[0]["content"]
                    except Exception:
                        comp_text = ""
                sent_labels = process_scores_per_sample[row_idx]  # list of 0/1
                # Skip if sent_labels is empty (no valid process scores)
                if not sent_labels or len(sent_labels) == 0:
                    continue
                single_accuracy_reward = accuracy_reward_list[row_idx] # float 0.0 ~ 1.0
                            ...
                            for idx, anchor_idx in enumerate(range(len(ids))):
                                ...
                                for pos in range(search_start, len(comp_ids_list)):
                                    ...
                                    if end_pos <= len(comp_ids_list) and comp_ids_list[pos:end_pos] == suffix:
                                        match_start = pos - idx
                                        if (len(ids) <= 2) or (len(ids) > 2 and (sent_idx + 1) == len(sentences_with_spans)):   # last token 제외
                                            match_end = end_pos
                                        elif len(ids) > 2:
                                            match_end = end_pos + 1
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # Log policy loss (before KL), masked and averaged
        ...
        if self.beta > 0:
            ...
            # Log KL loss contribution (beta * KL)
        ...
        # Log total loss explicitly for convenience
