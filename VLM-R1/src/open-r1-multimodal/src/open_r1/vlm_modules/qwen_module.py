...
class Qwen2VLModule(VLMBaseModule):
    def __init__(self):
        super().__init__()
        self.is_qwen3 = False  # Track if using Qwen3 model
    ...
    def get_custom_multimodal_keywords(self):
        if self.is_qwen3:
            return ['pixel_values', 'image_grid_thw', 'video_grid_thw']
    ...
    def prepare_prompt(self, processing_class, inputs: dict[str, Union[torch.Tensor, Any]], enable_thinking: bool = True):
        prompts_text = []
        for example in inputs:
            # For Qwen3, use apply_chat_template with enable_thinking
            if self.is_qwen3:
                if hasattr(processing_class, "apply_chat_template"):
                ...
                elif hasattr(getattr(processing_class, "tokenizer", None), "apply_chat_template"):
                    prompt = processing_class.tokenizer.apply_chat_template(
                        example["prompt"],
                        tokenize=False,
                        add_generation_prompt=True,
                        enable_thinking=enable_thinking,
                    ...
                    prompt = maybe_apply_chat_template(example, processing_class)["prompt"]
            else:
                # For Qwen2/Qwen2.5, use original behavior
                prompt = maybe_apply_chat_template(example, processing_class)["prompt"]
    ...
    def prepare_model_inputs(self, processing_class, prompts_text, images, return_tensors="pt", padding=True, padding_side="left", add_special_tokens=False):
        ...
        if len(images) > 0:
            ...
            # For Qwen3-VL, ensure video_grid_thw is explicitly set to None if not present
            if self.is_qwen3 and 'video_grid_thw' not in prompt_inputs:
                prompt_inputs['video_grid_thw'] = None
        else:
            prompt_inputs = processing_class(
                text=prompts_text,
                return_tensors=return_tensors,
